  ...
  try {
    ...
    try {
      const { data: recentChunks, error } = await supabase
        ...
        .select('id, chatbot_id, document_id, chunk_type, content_length, created_at')
    ...
    try {
      ...
      if (insertError) {
        ...
        console.log(`   ✅ Test chunk inserted successfully: ${insertResult[0].id}`);
    ...
    console.log('   3. Run the SQL schema from backend/database/supabase-chunk-schema.sql');
